...
// Corrected TaxRecord to match src/lib/types.ts
type TaxRecord = {
    id: string;
...
// Corrected Property to match src/lib/types.ts
type Property = {
    id: string;
    ownerName: string;
    fatherName: string;
...
export const generateBill = onCall(async (request) => {
    ...
    try {
        ...
        throw new HttpsError("internal", "An unexpected error occurred while generating the bill.", error.message);
    }
});
